    ...
    if (Array.isArray(val)) {
      ...
      return (val as unknown[])
        ...
        .map((item) => ({
          ...
          autor: typeof item.autor === 'string' ? item.autor : undefined,
          ...
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {lead.qualificada_ia && (
                <Badge
                  variant="outline"
                  className="text-xs px-2.5 py-0.5 border bg-emerald-50 text-emerald-800 border-emerald-300 font-bold flex items-center gap-1 shadow-xs"
                >
                  <Bot className="w-3 h-3 text-[#0B7A5B]" />
                  <span>Qualificada pela Amanda</span>
                </Badge>
              )}
              ...
              <div className="space-y-3 max-h-72 overflow-y-auto pr-1">
                {normalizedHistorico.length === 0 ? (
                  ...
                  [...normalizedHistorico].reverse().map((item, idx) => {
                    ...
                    const isAmanda =
                      item.autor === 'Amanda (IA)' ||
                      item.descricao?.toLowerCase().includes('amanda (ia)') ||
                      item.descricao?.toLowerCase().includes('pela amanda')
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          {isAmanda ? (
                            <Bot className="w-3.5 h-3.5 text-[#0B7A5B]" />
                          ) : isSlaAlert ? (
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-1.5 flex-wrap">
                            {isAmanda && (
                              <Badge className="bg-[#0B7A5B] text-white text-[9px] px-1.5 py-0 font-bold tracking-wide">
                                Amanda (IA)
                              </Badge>
                            )}
                  ...
                  waMessages.map((msg, idx) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          {isAmandaMsg && (
                            <div className="flex items-center gap-1 mb-1 text-[10px] text-emerald-200 font-bold tracking-wide">
