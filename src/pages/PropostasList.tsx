                          ...
                          <div className="pt-0.5 space-y-1">
                            {tracking.hasViewed ? (
                              <div className="space-y-1">
                                <div className="inline-flex flex-wrap items-center gap-2 text-xs text-amber-950 bg-amber-50 border border-amber-300 px-2.5 py-1 rounded-md shadow-2xs font-medium">
                                ...
                                {tracking.historico.length > 0 && (
                                  <div className="flex flex-wrap items-center gap-1 text-[10px] text-slate-600">
                                    <span className="text-slate-400 font-semibold">Horários:</span>
                                    {tracking.historico.slice(0, 3).map((vis, vIdx) => (
                                      <span
                                        key={`list-vis-${prop.id}-${vIdx}`}
                                        className="bg-slate-100 text-slate-700 px-1.5 py-0.5 rounded font-mono-numbers border border-slate-200"
                                        title={`Visualização #${tracking.historico.length - vIdx} em ${vis.formatado}`}
                                      >
                                        #{tracking.historico.length - vIdx}{' '}
                                        {vis.relativo || vis.formatado}
                                      </span>
                                    ))}
                                    {tracking.historico.length > 3 && (
                                      <span
                                        ...
                                        title={tracking.historico
                                          ...
                                          .map(
                                            (h, i) =>
                                              `#${tracking.historico.length - 3 - i}: ${h.formatado}`,
                                          )
                                          .join('\n')}
                                      >
                                        +{tracking.historico.length - 3} anteriores
                                      </span>
                                    )}
                                  </div>
                                )}
                              ...
                              <div className="inline-flex items-center gap-1 text-[11px] text-slate-400">
                                ...
                                <span>Não visualizada ainda</span>
                              </div>
                            )}
                          </div>
                        )
